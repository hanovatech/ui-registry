...
Components with internal labels (e.g., "Previous", "Save", month names) use a shared `t` store from `$lib/stores/i18n`. The `i18n` registry item provides a minimal store setup. If your project already has its own i18n implementation at the same paths, skip overwriting those files when prompted — the components are compatible with any implementation that exports a `t` store with the same shape.
